...
def draw_letter(letter:str, size:int, row:int, col:int, colour:tuple) -> list:
    ...
    pygame.draw.rect(screen, colour, rect, width=0, border_radius=7)
...
def draw_word(word:str,size:int, row:int,col: int, colour:tuple) -> list:
    ...
    for col,letter in enumerate(list(word)):
        rect = draw_letter(letter, size, row, col, colour)
        rects.append(rect)
    return rects
...
def touch_spot(touch_spots) -> str:
    for num,spots in enumerate(touch_spots):
        ...
        if left <= mouse[0] <= left + box_width and top <= mouse[1] <= top + box_height:
            pygame.draw.rect(screen,colour_light,[left, top, box_width, box_height],border_radius=7)
            letterfont = pygame.font.SysFont('Corbel',int(size*1.1))
            # for num,letter in enumerate(word):
            text = letterfont.render(letter,True,BLACK)
            text_rect = text.get_rect(center = ( left + box_width/2, top + box_height/2  ))
            screen.blit(text, text_rect)
            return letter

touch_spots = []
touch_spots = touch_spots + draw_word("WORDLE", 150, 1, 0, colour_dark)
touch_spots  = touch_spots + draw_word("INPUT_", 150, 2, 0, colour_dark)
# -------- Main Program Loop -----------
...
while carryOn:
    ...
    draw_word("WORDLE", 150, 1, 0, colour_dark)
    draw_word("INPUT_", 150, 2, 0, colour_dark)
